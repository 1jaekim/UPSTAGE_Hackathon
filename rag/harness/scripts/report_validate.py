...
reporter-subagent가 쓴 work/50_report.json에 대해:
  ...
  V-SCHEMA §5 필수 키·타입
  V-J2     exercises 정확히 3개 (특이사항으로 PT에게 위임된 경우엔 0개도 허용)
...
def main():
    ...
    if len(exercises) not in (0, 3):
        fails.append({"check": "V-J2", "detail": f"exercises count = {len(exercises)} (must be 0 or 3)"})
